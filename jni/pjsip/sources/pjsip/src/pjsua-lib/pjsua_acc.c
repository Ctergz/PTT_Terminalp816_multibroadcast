...
/*
 * jnicallback interface to call java.
 */
static void show_login_state(LOGIN_STATE state, int error_code)
{
    jmethodID showLoginId, constructorMethodId;
    ...
    showLoginId = (*jni_env)->GetMethodID(jni_env, jni_callback_class, "showLoginState", "(II)V");
    (*jni_env)->CallVoidMethod(jni_env, obj_jni_callback, showLoginId, state, error_code);
    DETACH_JVM(jni_env);
 ...
 * Clear account.
 */
PJ_DEF(pj_status_t) pjsua_acc_clear(void)
{
    unsigned i;
				    ...
				    struct pjsip_regc_cbparam *param)
{
	...
	matched = (uri->port == rport &&
		   ...
		   pj_ansi_strcpy(switch_addr, pj_inet_ntoa(recv_addr.ipv4.sin_addr));
    } else {
...
static void regc_cb(struct pjsip_regc_cbparam *param)
{
    ...
    pj_log_push_indent();
    PJ_LOG(3, ("zzc", "regcb registering"));
    registering = 0;
...
#ifdef PJ_ANDROID
    if (param->code == 200)
        show_login_state(REGISTERE_SUCCESS, param->code);
    else
        show_login_state(ERROR_CODE_NUMBER, param->code);
#endif
...
static pj_status_t pjsua_regc_init(int acc_id)
{
    ...
    /* Add Ptt-Extension: ipm header. */
    if(PJSIP_EXTEND_ZED == 1){
        pjsip_hdr hdr_list;
        ...
        hptt = pjsip_generic_string_hdr_create(pool, &STR_PTT_EXT, 
            &STR_PTT_EXT_VALUE);
        pj_list_push_back(&hdr_list, (pjsip_hdr*)hptt);
