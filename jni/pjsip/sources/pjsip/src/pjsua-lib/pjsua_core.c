					    ...
					    pjsua_transport_id *p_id)
{
    ...
    } else if (type == PJSIP_TRANSPORT_MULTICAST) {
	/*
...
/*
 * Get transport instance by id.
 */
PJ_DECL(pjsip_transport *) pjsua_get_transport(pjsua_transport_id id)
{
    return pjsua_var.tpdata[id].data.tp;
}
					   ...
					   pj_bool_t force )
{
    ...
    if (pjsua_var.tpdata[id].type == PJSIP_TRANSPORT_MULTICAST)
    {
        pjsua_var.tpdata[id].type = PJSIP_TRANSPORT_UDP;
    }
     ...
     *	     the transport.
     */
    PJ_LOG(3,(THIS_FILE, ">>>>pjsua_transport_close force %d, type %d", force, pjsua_var.tpdata[id].type));
    if (force) {
	switch (pjsua_var.tpdata[id].type) {
	case PJSIP_TRANSPORT_UDP:
	    status = pjsip_transport_shutdown(pjsua_var.tpdata[id].data.tp);
	    PJ_LOG(3,(THIS_FILE, ">>>>pjsip_transport_shutdown status %d", status));
	    if (status  != PJ_SUCCESS)
		return status;
	    status = pjsip_transport_destroy(pjsua_var.tpdata[id].data.tp);
	    PJ_LOG(3,(THIS_FILE, ">>>>pjsip_transport_destroy status %d", status));
	    if (status != PJ_SUCCESS)
		return status;
	    break;
