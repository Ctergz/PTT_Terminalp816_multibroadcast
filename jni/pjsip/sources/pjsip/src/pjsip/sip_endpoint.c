/* $Id: sip_endpoint.c 3753 2011-09-18 14:59:56Z bennylp $ */
/* 
				      ...
				      pjsip_rx_data *rdata )
{
    ...
    if (rdata->tp_info.transport->key.type == PJSIP_TRANSPORT_MULTICAST)
    {
        pjsip_module *mod;

        mod = endpt->module_list.prev;
        while (mod != &endpt->module_list) {
            if (!pj_strcmp2(&mod->name, "mod-multicast-handler"))
                (*mod->on_rx_request)(rdata);
            mod = mod->prev;
        }
        return;
    }
