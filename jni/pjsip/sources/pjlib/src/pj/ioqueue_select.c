...
#elif defined PJSIP_EXTEND_WOASIS

#include <linux/if.h>

static int getinterfacename(const char *dstip, char *interface_name)
{
    int i=0;
    int sockfd;
    ...
    for(i=(ifconf.ifc_len/sizeof(struct ifreq)); i>0; i--)
    {
        ...
        if(!strcmp(ip,dstip))
        {
           strcpy(interface_name,ifreq->ifr_name); 
            return 0;
        }
...
PJ_DEF(pj_status_t) sock_bind_device(pj_ioqueue_t *ioqueue, char *bind_addr)
{
    pj_ioqueue_key_t *h;
    pj_sock_t sock;
    ...
    pj_lock_acquire(ioqueue->lock);
    for (h = ioqueue->active_list.next; h!=&ioqueue->active_list; h = h->next)
    {
        sock = h->fd;

        memset(&if_eth, 0, sizeof(if_eth));
        if(!bind_addr || getinterfacename(bind_addr, if_eth.ifr_name) == 0)
            status = pj_sock_setsockopt(sock, SOL_SOCKET, SO_BINDTODEVICE, (char *)&if_eth, sizeof(if_eth));
        if (status != PJ_SUCCESS)
        {
            PJ_LOG(4,(THIS_FILE, "SO_BINDTODEVICE %s error %d!", if_eth.ifr_name, status));
            break;
        }
        else
        {
            PJ_LOG(4,(THIS_FILE, "SO_BINDTODEVICE %s success!", if_eth.ifr_name));
            continue;
        }
    }
    pj_lock_release(ioqueue->lock);
