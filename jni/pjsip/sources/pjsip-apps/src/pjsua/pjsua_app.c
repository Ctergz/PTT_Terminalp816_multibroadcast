...
//added by lxd
#define WOASIS_PTT_MSG_TYPE     "application"
#define WOASIS_PTT_MSG_SUBTYPE  "woasis-ptt"
...
unsigned 	sipcall_timer_count;//9* 3 seconds
int   registering = 0;
pj_log_func     *log_cb = NULL;
char switch_addr[PJ_INET_ADDRSTRLEN] = {0};
...
static void show_call_state(pjsua_call_id call_id, char *number, pjsip_inv_state inv_state, int error_code);
				  ...
				  struct pj_timer_entry *entry)
{
    ...
    if (call_info.remote_info.ptr)
    {
        char in_number[32] = {0};
        char *p1 = pj_ansi_strchr(call_info.remote_info.ptr, ':');
        char *p2 = pj_ansi_strchr(call_info.remote_info.ptr, '@');
        if (p1 && p2)
        {
            p1+=1;
            while(*p1 == ' ') p1++;
            pj_ansi_sprintf(in_number, "%.*s", p2-p1, p1);
        }
        else
        {
            pj_ansi_sprintf(in_number, "%.*s", call_info.remote_info.slen, call_info.remote_info.ptr);
        }
        show_call_state(call_id, in_number, PJSIP_INV_STATE_DISCONNECTED, 504);
    }
}
...
jclass jni_callback_class = NULL;
jclass call_state_class = NULL;
jclass group_info_class = NULL;
jclass member_info_class = NULL;
jclass ptt_group_status_class = NULL;

extern ptt_group_info * g_current_group;
static void show_call_state(pjsua_call_id call_id, char *number, pjsip_inv_state inv_state, int error_code)
{
    ENUM_CALL_STATE ecall_state;
    long seconds = 0;
    jmethodID constructorMethodId, showCallStateId, setCallIdId, setNumberId, setSecondsId, setCallStateId, setErrorCodeId;
    jobject obj_jni_callback, obj_call_state;
    jstring stringNumber = NULL;

    if (!jni_callback_class || !call_state_class)
        return;

    switch(inv_state)
    {
    case PJSIP_INV_STATE_NULL:
        ecall_state = CALL_IDLE;
        break;

    case PJSIP_INV_STATE_CALLING:
        ecall_state = CALL_DAILING;
        break;
    ...
    case PJSIP_INV_STATE_INCOMING:
        ...
        else if (ptt_get_state() == PTT_STATUS_BUSY)
        {
            if(group_number_compare(number))
                ptt_close_media();
            else
                return;
        }
        if (ptt_get_state() == PTT_STATUS_FREE)
        {
            ptt_close_media();
        }
		pj_ansi_strcpy(dest_num, number);
        ecall_state = CALL_RINGING;
        break;

    case PJSIP_INV_STATE_CONFIRMED:
        ecall_state = CALL_TALKING;
        break;
    ...
    case PJSIP_INV_STATE_DISCONNECTED:
    {
        char tmp[64] = {0};
        if(g_current_group)
            PJ_LOG(3, (THIS_FILE, ">>>>>>DISCONNECTED current_call %d call_id %d pttstate %d current_grp %.*s dest %s", current_call, call_id, ptt_get_state(), g_current_group->number.slen, g_current_group->number.ptr, dest_num));
        else
            PJ_LOG(3, (THIS_FILE, ">>>>>>DISCONNECTED current_call %d call_id %d pttstate %d dest %s", current_call, call_id, ptt_get_state(), dest_num));
        if (current_call != PJSUA_INVALID_ID && current_call != call_id)
            return;
        pjsua_set_no_snd_dev();
        pj_ansi_strcpy(tmp, dest_num);
		pj_bzero(dest_num, sizeof(dest_num));
        if (ptt_get_state() == PTT_STATUS_BUSY && pj_ansi_strncmp(tmp, g_current_group->number.ptr, g_current_group->number.slen))
            return;
        ecall_state = CALL_ENDING;
        break;
    }
    default:
        return;
    ...
    constructorMethodId = (*jni_env)->GetMethodID(jni_env, call_state_class, "<init>", "()V");
    obj_call_state = (*jni_env)->NewObject(jni_env, call_state_class, constructorMethodId);

    showCallStateId = (*jni_env)->GetMethodID(jni_env, jni_callback_class, "showCallState", "(Lcom/woasis/ptt/model/CallState;)V");

    setCallIdId = (*jni_env)->GetMethodID(jni_env, call_state_class, "setCallId", "(I)V");
    setNumberId = (*jni_env)->GetMethodID(jni_env, call_state_class, "setNumber", "(Ljava/lang/String;)V");
    //setSecondsId = (*jni_env)->GetMethodID(jni_env, call_state_class, "setSeconds", "(L)V");
    setCallStateId = (*jni_env)->GetMethodID(jni_env, call_state_class, "setCallState", "(I)V");
    setErrorCodeId = (*jni_env)->GetMethodID(jni_env, call_state_class, "setErrorCode", "(I)V");

    if (number)
        stringNumber = (*jni_env)->NewStringUTF(jni_env, number);
    (*jni_env)->CallVoidMethod(jni_env, obj_call_state, setCallIdId, call_id);
    (*jni_env)->CallVoidMethod(jni_env, obj_call_state, setNumberId, stringNumber);
    //(*jni_env)->CallVoidMethod(jni_env, obj_call_state, setSecondsId, seconds);
    (*jni_env)->CallVoidMethod(jni_env, obj_call_state, setCallStateId, ecall_state);
    (*jni_env)->CallVoidMethod(jni_env, obj_call_state, setErrorCodeId, error_code);

    (*jni_env)->CallVoidMethod(jni_env, obj_jni_callback, showCallStateId, obj_call_state);
...
static void on_call_state(pjsua_call_id call_id, pjsip_event *e)
{
    ...
    if (call_info.remote_info.ptr)
    {
        ...
        if(call_info.state != PJSIP_INV_STATE_INCOMING)
            show_call_state(call_id, in_number, call_info.state, call_info.last_status);
    }
			     ...
			     pjsip_rx_data *rdata)
{
    ...
    if(g_current_group)
        PJ_LOG(3, (THIS_FILE, ">>>>>>on_incoming_call call_id %d, current_call %d, pttstate %d dest %s g_current_group %.*s in_num %s", call_id, current_call, ptt_get_state(), dest_num, g_current_group->number.slen, g_current_group->number.ptr, in_number));
    else
        PJ_LOG(3, (THIS_FILE, ">>>>>>on_incoming_call call_id %d, current_call %d, pttstate %d dest %s in_num %s", call_id, current_call, ptt_get_state(), dest_num, in_number));        
        
    if(current_call != call_id || ptt_get_state() == PTT_STATUS_BUSY)
    {
        if(g_current_group && group_number_compare(in_number))
        {
            ptt_close_media();
        }
        else
...
static void on_reg_state2(pjsua_acc_id acc_id, pjsua_reg_info *info)
{
    ...
    pjsip_msg *msg;
    const pj_str_t STR_PTT_EXT = { "Ptt-Extension", 13 };
    const pj_str_t STR_PTT_EXT_VALUE = { "ipm", 3 };
    pjsip_generic_string_hdr *hptt;

    PJ_UNUSED_ARG(acc_id);

    if (PJSIP_EXTEND_ZED != 1 || !info->cbparam->rdata || info->cbparam->expiration == 0)
    {
        return;
    }
    msg = info->cbparam->rdata->msg_info.msg;
    hptt = pjsip_msg_find_hdr_by_name(msg, &STR_PTT_EXT, NULL);
    if (!hptt || pj_strcmp(&hptt->hvalue, &STR_PTT_EXT_VALUE))
    {
        return;
    }
    PJ_LOG(4, (THIS_FILE, "zzc\nPtt-Extension: %.*s\n", hptt->hvalue.slen, hptt->hvalue.ptr));

    if(msg->body)
        ptt_reg200ok_content_parse(msg->body->data, msg->body->len);
#endif
}
...
{
    pj_bool_t b_group_change = PJ_FALSE;
...
#ifdef PJSIP_EXTEND_WOASIS
    if(!pj_ansi_strncmp("text/plain" , mime_type->ptr, mime_type->slen))
    {
       // deal as sms
    ...
    else if(!pj_ansi_strncmp("application/woasis-ptt" , mime_type->ptr, mime_type->slen))
    {
       ...
       if(!pj_ansi_strncmp("ind:groupInfo\r\n", body->ptr, 15))
       {
    ...
    if (PJSIP_EXTEND_ZED == 1 && rdata)
    {
        msg = rdata->msg_info.msg;
        hptt = pjsip_msg_find_hdr_by_name(msg, &STR_PTT_EXT, NULL);
        if (hptt && !pj_strcmp(&hptt->hvalue, &STR_PTT_EXT_VALUE) && body)
        {
            b_group_change = PJ_TRUE;
        }
    }
    ...
    {
        stringNumber = (*jni_env)->NewStringUTF(jni_env, from_number);
        stringText = (*jni_env)->NewStringUTF(jni_env, body->ptr);
...
JNIEXPORT jint JNICALL login(JNIEnv* env, jclass cls, jobject server, jobject user)
{
    ...
    jclass clazzServer = (*env)->GetObjectClass(env, server);
    jclass clazzUser = (*env)->GetObjectClass(env, user);
    jfieldID field_server_ip = (*env)->GetFieldID(env, clazzServer, "serverIp", "Ljava/lang/String;");
    jfieldID field_port = (*env)->GetFieldID(env, clazzServer, "port", "Ljava/lang/String;");
    jfieldID field_username = (*env)->GetFieldID(env, clazzUser, "username", "Ljava/lang/String;");
    jfieldID field_passwd = (*env)->GetFieldID(env, clazzUser, "passwd", "Ljava/lang/String;");

    jstring str_server_ip = (*env)->GetObjectField(env, server, field_server_ip);
    jstring str_port = (*env)->GetObjectField(env, server, field_port);
    jstring str_username = (*env)->GetObjectField(env, user, field_username);
    jstring str_passwd = (*env)->GetObjectField(env, user, field_passwd);

    utf_chars = (char *)(*env)->GetStringUTFChars(env, str_server_ip, 0);
    pj_ansi_strcpy(sip_server.serverip, utf_chars);
    (*env)->ReleaseStringUTFChars(env, str_server_ip, utf_chars);
    utf_chars = (char *)(*env)->GetStringUTFChars(env, str_port, 0);
    pj_ansi_strcpy(sip_server.port, utf_chars);
    (*env)->ReleaseStringUTFChars(env, str_port, utf_chars);
    utf_chars = (char *)(*env)->GetStringUTFChars(env, str_username, 0);
    pj_ansi_strcpy(sip_user.username, utf_chars);
    (*env)->ReleaseStringUTFChars(env, str_username, utf_chars);
    utf_chars = (char *)(*env)->GetStringUTFChars(env, str_passwd, 0);
    pj_ansi_strcpy(sip_user.passwd, utf_chars);
    (*env)->ReleaseStringUTFChars(env, str_passwd, utf_chars);

    pj_ansi_sprintf(id, "sip:%s@%s", sip_user.username, sip_server.serverip);
    if (pj_ansi_strcmp(sip_server.port, "5060"))
        pj_ansi_sprintf(registrar, "sip:%s:%s", sip_server.serverip, sip_server.port);
    else
        pj_ansi_sprintf(registrar, "sip:%s", sip_server.serverip);
    pj_ansi_strcpy(uname, sip_user.username);
    pj_ansi_strcpy(passwd, sip_user.passwd);
    ...
    acc_cfg.cred_count = 1;
    acc_cfg.cred_info[0].scheme = pj_str("Digest");
    acc_cfg.cred_info[0].realm = pj_str("*");//zzc modify
    acc_cfg.cred_info[0].username = pj_str(uname);
    acc_cfg.cred_info[0].data_type = 0;
    acc_cfg.cred_info[0].data = pj_str(passwd);
    //added by lxd
    acc_cfg.reg_timeout = PTT_REG_INTERVAL;

    acc_cfg.rtp_cfg = app_config.rtp_cfg;
    app_config_init_video(&acc_cfg);

    pj_memset(switch_addr, 0, sizeof(switch_addr));
    pjsua_acc_clear();
    registering = 1;
    ptt_destroy();
    pj_thread_sleep(200);

    ioqueue = pjsip_endpt_get_ioqueue(pjsua_get_pjsip_endpt());
    sock_bind_device(ioqueue, NULL);
    ioqueue = pjmedia_endpt_get_ioqueue(pjsua_get_pjmedia_endpt());
    sock_bind_device(ioqueue, NULL);

    ptt_init();
    status = pjsua_acc_add(&acc_cfg, PJ_TRUE, NULL);
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Error add account", status);
    }

    return status;
}

JNIEXPORT jint JNICALL make_call(JNIEnv* env, jclass cls, jobject numberInfo)
{
    pjsua_call_setting call_opt;
    pjsua_msg_data msg_data;
    number_info_t tnumber_info;
    pjsua_acc_info info;
    pj_str_t t_tmp;
    char *utf_chars;
    char tmp[64];

    jclass clazzNumberInfo = (*env)->GetObjectClass(env, numberInfo);
    jfieldID field_number_type = (*env)->GetFieldID(env, clazzNumberInfo, "numberType", "I");
    jfieldID field_number = (*env)->GetFieldID(env, clazzNumberInfo, "number", "Ljava/lang/String;");
    jfieldID field_sipuri = (*env)->GetFieldID(env, clazzNumberInfo, "sipUri", "Ljava/lang/String;");

    tnumber_info.number_type = (*env)->GetIntField(env, numberInfo, field_number_type);

    jstring str_number = (*env)->GetObjectField(env, numberInfo, field_number);
    jstring str_sipuri = (*env)->GetObjectField(env, numberInfo, field_sipuri);

    if (tnumber_info.number_type == DIGIT)
    {
        utf_chars = (char *)(*env)->GetStringUTFChars(env, str_number, 0);
        pj_ansi_strcpy(tnumber_info.number, utf_chars);
		pj_ansi_strcpy(dest_num, utf_chars);
        (*env)->ReleaseStringUTFChars(env, str_number, utf_chars);
    }
    else
    {
        utf_chars = (char *)(*env)->GetStringUTFChars(env, str_sipuri, 0);
        pj_ansi_strcpy(tnumber_info.sip_uri, utf_chars);
        (*env)->ReleaseStringUTFChars(env, str_sipuri, utf_chars);
    }

    if (current_call != PJSUA_INVALID_ID)
    {
        if(PJSIP_ESESSIONTERMINATED == pjsua_call_hangup(current_call, 0, NULL, NULL))
        {
            current_call = PJSUA_INVALID_ID;
        }
    }
    ...
    if (tnumber_info.number_type == DIGIT)
    {
        if (pj_ansi_strchr(info.acc_uri.ptr, '@'))
            pj_ansi_sprintf(tmp, "sip:%s%s", tnumber_info.number, pj_ansi_strchr(info.acc_uri.ptr, '@'));
        else
            return PJ_EINVAL;
    }
    else if(tnumber_info.number_type == SIPURI)
    {
        pj_ansi_sprintf(tmp, "%s", tnumber_info.sip_uri);
    }

    ptt_close_media();

    t_tmp = pj_str(tmp);
    return pjsua_call_make_call(current_acc, &t_tmp, &call_opt, NULL, &msg_data, NULL);
}
...
JNIEXPORT jint JNICALL answer(JNIEnv* env, jclass cls, jint call_id)
{
    ...
    {
        pjsua_msg_data_init(&msg_data);
        return pjsua_call_answer2(call_id, &call_opt, 200, NULL, &msg_data);
    }
}

JNIEXPORT jint JNICALL hangup(JNIEnv* env, jclass cls, jint call_id)
{
    return pjsua_call_hangup(call_id, 0, NULL, NULL);
}

void get_dest_number(pj_str_t *number)
{
    number->ptr = dest_num;
    number->slen = pj_ansi_strlen(dest_num);
}

static void info_tsx_callbac(void *token, pjsip_event *event)
{
    pjsip_msg *msg;
    pjsip_rx_data *rdata;
    const pj_str_t STR_PTT_EXT = { "Ptt-Extension", 13 };
    ...
    PJ_UNUSED_ARG(token);//it is NULL when call pjsip_endpt_send_request.
    if(event->type == PJSIP_EVENT_TSX_STATE && event->body.tsx_state.type == PJSIP_EVENT_RX_MSG)
    {
        PJ_LOG(4,(THIS_FILE, "===================zzc continue"));
    }
    else
    {
        PJ_LOG(4,(THIS_FILE, "===================zzc return event&tsx type:%d&%d", event->type, event->body.tsx_state.type));
        return;
    }
    rdata = event->body.tsx_state.src.rdata;
    msg = rdata->msg_info.msg;

    if (msg->line.status.code/100 != 2 && msg->line.status.code != PJSIP_SC_FORBIDDEN)
    {
        pjsua_perror(THIS_FILE, "Error status code", msg->line.status.code);
        return;
    }
    hptt = pjsip_msg_find_hdr_by_name(msg, &STR_PTT_EXT, NULL);
#ifdef PJSIP_EXTEND_ZED
    if (PJSIP_EXTEND_ZED != 1 || !hptt || pj_strcmp(&hptt->hvalue, &STR_PTT_EXT_VALUE))
    {
        pjsua_perror(THIS_FILE, "Not ptt info!", PJSIP_EXTEND_ZED);
        return;
    }
#endif
    if(msg->body)
    {
        ptt_info200ok_content_parse(msg->body->data, msg->body->len);
    }
    else if (msg->line.status.code == PJSIP_SC_FORBIDDEN)
    {
        ptt_info200ok_content_parse(NULL, PJSIP_SC_FORBIDDEN);
    }
}

#ifdef PJSIP_EXTEND_WOASIS
static void msg_tsx_callbac(void *token, pjsip_event *event);
static pj_status_t send_ptt_message(msg_action action, char *number)
{
    pj_str_t str_method;
    pjsip_method method;
    pjsua_acc_info info;
    ...
    if (pj_ansi_strchr(info.acc_uri.ptr, '@'))
    {
        //utf_chars = (char *)(*env)->GetStringUTFChars(env, num, 0);
        pj_ansi_sprintf(tmp, "sip:%s%s", number, pj_ansi_strchr(info.acc_uri.ptr, '@'));
        //(*env)->ReleaseStringUTFChars(env, num, utf_chars);
    }
    else
    {
        return PJ_ENOTFOUND;
    ...
    type = pj_str(WOASIS_PTT_MSG_TYPE);
    subtype = pj_str(WOASIS_PTT_MSG_SUBTYPE);
    ...
    PJ_LOG(3, (THIS_FILE, "send_ptt_message : %s", msg));
...
//info_body can be NULL
#ifdef PJSIP_EXTEND_ZED
static pj_status_t send_ptt_info(char *dest_num, char *info_body)
{
    pj_status_t status;

    pj_str_t str_method, dst_uri, type, subtype, body;
    pjsip_method method;
    pjsip_endpoint *endpt;
    pjsua_acc_info info;
    pjsip_tx_data *tdata;
    ...
    endpt = pjsua_get_pjsip_endpt();
    str_method = pj_str("INFO");
    pjsip_method_init_np(&method, &str_method);
    pjsua_acc_get_info(current_acc, &info);
    if (pj_ansi_strchr(info.acc_uri.ptr, '@'))    
        pj_ansi_sprintf(tmp, "sip:%s%s", dest_num, pj_ansi_strchr(info.acc_uri.ptr, '@'));
    else
        return PJ_ENOTFOUND;
    ...
    hptt = pjsip_generic_string_hdr_create(tdata->pool, NULL, NULL);
    hptt->name = pj_str("Ptt-Extension");
    hptt->hvalue = pj_str("ipm");
    pjsip_msg_add_hdr(tdata->msg, (pjsip_hdr*)hptt);

    if (info_body)
    {
        type = pj_str("text");
        subtype = pj_str("ipm");
        body = pj_str(info_body);
        /* Add message body */
        tdata->msg->body = pjsip_msg_body_create( tdata->pool, &type, &subtype, &body);
    ...
    status = pjsip_endpt_send_request(endpt, tdata, -1, NULL, &info_tsx_callbac);
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Unable to send request", status);
        return status;
    }
...
JNIEXPORT jint JNICALL get_group_member(JNIEnv* env, jclass cls, jstring num)
{
    ...
    utf_chars = (char *)(*env)->GetStringUTFChars(env, num, 0);
#ifdef PJSIP_EXTEND_ZED
    status = send_ptt_info(utf_chars, "ipm-req: getstatus");
#endif
#ifdef PJSIP_EXTEND_WOASIS
    status = send_ptt_message(ACTION_GET_MEMBER, utf_chars);
#endif
...
JNIEXPORT jint JNICALL get_group_number(JNIEnv* env, jclass cls, jstring num)
{
    ...
    utf_chars = (char *)(*env)->GetStringUTFChars(env, num, 0);
    status = send_ptt_message(ACTION_GET_GRP_NUMBER, utf_chars);
    (*env)->ReleaseStringUTFChars(env, num, utf_chars);
...
JNIEXPORT jint JNICALL apply_ptt_right(JNIEnv* env, jclass cls, jstring num)
{
    ...
    utf_chars = (char *)(*env)->GetStringUTFChars(env, num, 0);
#ifdef PJSIP_EXTEND_ZED
    status = send_ptt_info(utf_chars, "ipm-req: request");
#endif
#ifdef PJSIP_EXTEND_WOASIS
    status = send_ptt_message(ACTION_APPLY_PTT_RIGHT, utf_chars);
#endif
    (*env)->ReleaseStringUTFChars(env, num, utf_chars);
...
JNIEXPORT jint JNICALL cancel_ptt_right(JNIEnv* env, jclass cls, jstring num)
{
    ...
    utf_chars = (char *)(*env)->GetStringUTFChars(env, num, 0);
#ifdef PJSIP_EXTEND_ZED
    status = send_ptt_info(utf_chars, "ipm-req: cancel");
#endif
#ifdef PJSIP_EXTEND_WOASIS
    status = send_ptt_message(ACTION_CANCEL_PTT_RIGHT, utf_chars);
#endif
    (*env)->ReleaseStringUTFChars(env, num, utf_chars);
...
static void msg_tsx_callbac(void *token, pjsip_event *event)
{
    ...
    int status_code;
    pjsip_rx_data *rdata;
    jmethodID msgResultId, constructorMethodId;
    jobject obj_jni_callback;
    JNIEnv *jni_env = 0;
    ...
    constructorMethodId = (*jni_env)->GetMethodID(jni_env, jni_callback_class, "<init>", "()V");
    obj_jni_callback = (*jni_env)->NewObject(jni_env, jni_callback_class, constructorMethodId);
    msgResultId = (*jni_env)->GetMethodID(jni_env, jni_callback_class, "sendMessageResult", "(I)V");
    (*jni_env)->CallVoidMethod(jni_env, obj_jni_callback, msgResultId, status_code);
    DETACH_JVM(jni_env);
}
...
JNIEXPORT jint JNICALL send_message(JNIEnv* env, jclass cls, jstring num, jstring text)
{
    ...
    {
        utf_chars = (char *)(*env)->GetStringUTFChars(env, num, 0);
        pj_ansi_sprintf(tmp, "sip:%s%s", utf_chars, pj_ansi_strchr(info.acc_uri.ptr, '@'));
        (*env)->ReleaseStringUTFChars(env, num, utf_chars);
    }
    else
    {
    ...
    type = pj_str("text");
    subtype = pj_str("plain");
    utf_chars = (char *)(*env)->GetStringUTFChars(env, text, 0);
    pj_ansi_strcpy(msg, utf_chars);
    (*env)->ReleaseStringUTFChars(env, text, utf_chars);
    body = pj_str(msg);
    /* Add message body */
    tdata->msg->body = pjsip_msg_body_create( tdata->pool, &type, &subtype, &body);
    ...
    status = pjsip_endpt_send_request(endpt, tdata, -1, NULL, &msg_tsx_callbac);
    if (status != PJ_SUCCESS) {
        pjsua_perror(THIS_FILE, "Unable to send request", status);
        return status;
    }
...
JNIEXPORT jint JNICALL ho_ind(JNIEnv* env, jclass cls, jstring ip)
{
    ...
    utf_chars = (char *)(*env)->GetStringUTFChars(env, ip, 0);

    manage_ptt_group(IP_DROP_MEMBERSHIP);
    if(strcmp(switch_addr, utf_chars))
    {
        PJ_LOG(4, (THIS_FILE, "ho ip: %s!", utf_chars));
        pj_ansi_strcpy(switch_addr, utf_chars);
    }
    else
    {
        PJ_LOG(4, (THIS_FILE, "same address!"));
        manage_ptt_group(IP_ADD_MEMBERSHIP);
        return PJ_SUCCESS;
    }

    ioqueue = pjsip_endpt_get_ioqueue(pjsua_get_pjsip_endpt());
    sock_bind_device(ioqueue, utf_chars);
    ioqueue = pjmedia_endpt_get_ioqueue(pjsua_get_pjmedia_endpt());
    sock_bind_device(ioqueue, utf_chars);
    if(pjsua_call_get_count() > 0)
    {
    ...
    manage_ptt_group(IP_ADD_MEMBERSHIP);
    pjsua_acc_set_registration(current_acc, PJ_TRUE);
    PJ_LOG(4, (THIS_FILE, "ho finish!"));

    (*env)->ReleaseStringUTFChars(env, ip, utf_chars);
...
pj_status_t app_main(void)
{
...
#if (PJSIP_EXTEND_ZED || PJSIP_EXTEND_WOASIS)
    ...
    if (!jni_callback_class)
    {
        jni_callback_class = (*jni_env)->NewGlobalRef(jni_env, (*jni_env)->FindClass(jni_env, "com/woasis/ptt/jni/JniCallback"));
        if (jni_callback_class == NULL) {
            PJ_LOG(2, (THIS_FILE, "Not able to find JniCallback class"));
        }
    }
    if (!call_state_class)
    {
        call_state_class = (*jni_env)->NewGlobalRef(jni_env, (*jni_env)->FindClass(jni_env, "com/woasis/ptt/model/CallState"));
        if (call_state_class == NULL) {
            PJ_LOG(2, (THIS_FILE, "Not able to find CallState class"));
        }
    }
    if (!group_info_class)
    {
        group_info_class = (*jni_env)->NewGlobalRef(jni_env, (*jni_env)->FindClass(jni_env, "com/woasis/ptt/model/GroupInfo"));
        if (group_info_class == NULL) {
            PJ_LOG(2, (THIS_FILE, "Not able to find GroupInfo class"));
        }
    }
    if (!member_info_class)
    {
        member_info_class = (*jni_env)->NewGlobalRef(jni_env, (*jni_env)->FindClass(jni_env, "com/woasis/ptt/model/MemberInfo"));
        if (member_info_class == NULL) {
            PJ_LOG(2, (THIS_FILE, "Not able to find MemberInfo class"));
        }
    }
    if (!ptt_group_status_class)
    {
        ptt_group_status_class = (*jni_env)->NewGlobalRef(jni_env, (*jni_env)->FindClass(jni_env, "com/woasis/ptt/model/PttGroupStatus"));
        if (ptt_group_status_class == NULL) {
            PJ_LOG(2, (THIS_FILE, "Not able to find PttGroupStatus class"));
        }
    }
    DETACH_JVM(jni_env);
...
pj_status_t app_destroy(void)
{
    ...
    if (jni_callback_class)
    {
        (*jni_env)->DeleteGlobalRef(jni_env, jni_callback_class);
        jni_callback_class = NULL;

    }
    if (call_state_class)
    {
        (*jni_env)->DeleteGlobalRef(jni_env, call_state_class);
        call_state_class = NULL;

    }
    if (group_info_class)
    {
        (*jni_env)->DeleteGlobalRef(jni_env, group_info_class);
        group_info_class = NULL;

    }
    if (member_info_class)
    {
        (*jni_env)->DeleteGlobalRef(jni_env, member_info_class);
        member_info_class = NULL;

    }
    if (ptt_group_status_class)
    {
        (*jni_env)->DeleteGlobalRef(jni_env, ptt_group_status_class);
        ptt_group_status_class = NULL;
